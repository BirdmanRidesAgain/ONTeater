...
print_help <- function() {
  ...
  Options: 
          ...
          --output_csv: This option outputs a csv of all filtered/plotted alignments. Turned on by default.
          ")
...
# Read in PAF
  ...
  if (args$raw == FALSE) {
    ...
    # Reasonable for most chromosomal-level arrangementsl; may exclude a couple of micro
    # We can optimize later.
    paf <- paf %>%
      dplyr::filter(tp == "P" & alen >= min_clean_aln_len & mapq > 40)
  }
