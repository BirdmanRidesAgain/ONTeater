...
validate_prefix() {
  ...
  if [[ "$WORKFLOW_MODE" == "postprocess" || "$WORKFLOW_MODE" == "run" ]]; then
    ...
    compgen -G "$asm_dir/*_major_merged.fa" >/dev/null || { echo "[smoke] ERROR: missing merged assembly under $asm_dir" >&2; return 1; }
    compgen -G "$asm_dir/*_major_merged_purged.fa" >/dev/null || { echo "[smoke] ERROR: missing purged assembly under $asm_dir" >&2; return 1; }
  fi
  if [[ "$WORKFLOW_MODE" == "qc" || "$WORKFLOW_MODE" == "run" ]]; then
    local asm_dir="$root/assemblies"
    compgen -G "$asm_dir/*_final_quast_report.txt" >/dev/null || { echo "[smoke] ERROR: missing QUAST report under $asm_dir" >&2; return 1; }
    compgen -G "$asm_dir/*_final_compleasm_report.txt" >/dev/null || { echo "[smoke] ERROR: missing Compleasm report under $asm_dir" >&2; return 1; }
  fi
}
